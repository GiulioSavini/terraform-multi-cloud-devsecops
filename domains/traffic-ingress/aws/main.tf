# -----------------------------------------------------------------------------
# AWS Ingress Module
# AWS Load Balancer Controller, external-dns, and cert-manager
# with Let's Encrypt ClusterIssuer.
# -----------------------------------------------------------------------------
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${local.oidc_id}:sub" = "system:serviceaccount:external-dns:external-dns"
            "${local.oidc_id}:aud" = "sts.amazonaws.com"
          }
        }
      }
